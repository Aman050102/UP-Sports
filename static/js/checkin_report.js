...
async function fetchRows() {
  ...
  return data.map((r) => ({
    ...r,
    action: r.action || (r.facility === "pool" ? "in" : "in"),
  }));
}
...
function updateTableHeader() {
  ...
  if (facilityFilter === "pool") {
    thead.innerHTML =
      "<tr><th>เวลาเข้า</th><th>เวลาออก</th><th>วันที่ (session)</th><th>สนาม</th></tr>";
  ...
  if (facilityFilter === "pool") {
    ...
    chart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [
          {
            label: `จำนวนเข้าใช้ (${labelForMode(timeMode)})`,
            data: dataIn,
...
$("#btnDoc").addEventListener("click", async () => {
  ...
  const head = ["วันที่ (session)", "ชื่อสนาม", "จำนวนคนเข้าใช้"].map(
    ...
    trs.push(
      new TableRow({
        children: [
          ...
          new TableCell({
            children: [new Paragraph(String(r["จำนวนคนเข้าใช้"]))],
          }),
        ],
      }),
    );
